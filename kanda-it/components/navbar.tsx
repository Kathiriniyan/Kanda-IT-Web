...
export interface StaggeredMenuProps {
  ...
  // ✅ logo control
  showLogo?: boolean; // show logo always (default true)
  showLogoWhenOpen?: boolean; // show logo only when menu open
  logoOpenDelayMs?: number; // ✅ delay for logo when opening
...
// ------------------------------
// ReactBits StaggeredMenu (INLINE)
// ------------------------------
const StaggeredMenuInline: React.FC<StaggeredMenuProps> = ({
  position = "right",
  ...
  const logoDelayTweenRef = useRef<gsap.core.Tween | null>(null);

  useEffect(() => {
    if (!showLogoWhenOpen) return;

    logoDelayTweenRef.current?.kill();
    logoDelayTweenRef.current = null;

    if (open) {
      setShowDelayedLogo(false);
      logoDelayTweenRef.current = gsap.delayedCall(Math.max(0, logoOpenDelayMs) / 1000, () => {
  ...
  }, [open, showLogoWhenOpen, logoOpenDelayMs]);
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const socialLinksEls = Array.from(panel.querySelectorAll(".sm-socials-link")) as HTMLElement[];
    ...
    if (socialTitle || socialLinksEls.length) {
      ...
      if (socialLinksEls.length) {
        tl.to(
          socialLinksEls,
          {
            ...
            onComplete: () => {
              gsap.set(socialLinksEls, { clearProps: "opacity" });
            },
          },
          socialsStart + 0.04
        );
  ...
  const shouldRenderLogo = showLogoWhenOpen ? showDelayedLogo : showLogo;
      ...
      >
        ...
        <header className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-[2em] bg-transparent pointer-events-none z-20">
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo">
            {shouldRenderLogo ? (
              <Image src={logoUrl} alt="Logo" className="sm-logo-img block h-8 w-auto object-contain" draggable={false} width={110} height={24} />
            ) : null}
          </div>
          ...
          <button
            ...
            type="button"
          >
            <span ref={textWrapRef} className="sm-toggle-textWrap relative inline-block h-[1em] overflow-hidden whitespace-nowrap">
...
.sm-scope .sm-logo { display: flex; align-items: center; user-select: none; pointer-events: auto; }
...
export default function Navbar() {
  ...
  const TRIGGER_Y = 140;
  const ANIM_IN_DUR = 0.6;

  const stickyRef = useRef<HTMLElement | null>(null);
  const stickyTween = useRef<gsap.core.Tween | null>(null);

  // ✅ Keep ALL links for mobile menu (includes Contact)
  const menuItems: StaggeredMenuItem[] = useMemo(() => links.map((l) => ({ label: l.name, ariaLabel: `Go to ${l.name} page`, link: l.href })), []);

  // ✅ Desktop nav links: remove Contact ONLY there
  const desktopLinks = useMemo(() => links.filter((l) => l.href !== "/contact"), []);

  const socials: StaggeredMenuSocialItem[] = useMemo(() => socialLinks.map((s) => ({ label: s.label, link: s.link })), []);
  ...
  useEffect(() => {
    ...
    stickyTween.current?.kill();

    if (showSticky) {
      gsap.set(el, { pointerEvents: "auto" });
      stickyTween.current = gsap.fromTo(el, { y: -24, opacity: 0 }, { y: 0, opacity: 1, duration: ANIM_IN_DUR, ease: "power3.out" });
    } else {
      stickyTween.current = gsap.to(el, {
        y: -24,
        opacity: 0,
        duration: 0.35,
        ease: "power2.in",
  ...
  return (
    <>
      <div className="absolute top-0 left-0 w-full z-50">
        <div className="hidden md:block bg-[#ff6a3d]">
          <div className="relative h-[44px] px-6 md:px-16 lg:px-24 xl:px-32 flex items-center">
            <div className="flex items-center gap-3 text-sm text-white font-medium">
              <span>Are you ready to grow up your business ?</span>

              <Link href="/contact" className="inline-flex items-center gap-2 hover:opacity-90 transition-opacity">
        ...
        <nav className="px-4 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-neutral-200/70 bg-white/40 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <Link href="/" className="inline-flex items-center">
              <Image src="/assets/logo.svg" alt="Buildify Logo" width={135} height={36} />
            </Link>

            {/* ✅ desktop links (Contact removed) */}
            <div className="hidden lg:flex gap-3 items-center">
              {desktopLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="navfx group relative inline-block py-2 px-3"
                  aria-label={`Go to ${link.name} page`}
                >
                  <span className="navfx-inner relative inline-block h-[1.05em] overflow-hidden leading-none">
                    <span className="navfx-line navfx-line--base block translate-y-0 text-neutral-900">
                      {link.name}
                    </span>
                    <span className="navfx-line navfx-line--hover block text-[#fe6a3d]">
                      {link.name}
                    </span>
                  </span>

                  <span className="navfx-underline absolute left-3 right-3 bottom-[6px] h-[2px] origin-left scale-x-0 bg-[#fe6a3d]" />
                </Link>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/contact"
                className={cx(
                  "hidden lg:inline-flex items-center gap-2 rounded-full",
                  "py-2.5 px-6 text-white bg-orange-500",
                  "shadow-[inset_0_2px_4px_rgba(255,255,255,0.55)]",
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link href="/" className="inline-flex items-center">
            <Image src="/assets/logo.svg" alt="Buildify Logo" width={120} height={32} />
          </Link>

          {/* ✅ sticky desktop links (Contact removed) */}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className={cx(
                "hidden lg:inline-flex items-center gap-2 rounded-full",
                "py-2 px-5 text-white bg-orange-500",
                "shadow-[inset_0_2px_4px_rgba(255,255,255,0.55)]",
                "transition-all duration-300 group"
              )}
            >
      ...
      {/* ✅ ONLY NEW: Desktop nav hover animation styles */}
      <style>{`
        /* Nav hover: text flip + underline grow (left->right) */
        .navfx .navfx-line {
          transition: transform 420ms cubic-bezier(0.22, 1, 0.36, 1);
          will-change: transform;
        }
        .navfx .navfx-line--hover {
          transform: translateY(110%);
        }
        .navfx:hover .navfx-line--base {
          transform: translateY(-110%);
        }
        .navfx:hover .navfx-line--hover {
          transform: translateY(-100%);
        }

        .navfx .navfx-underline {
          transition: transform 520ms cubic-bezier(0.22, 1, 0.36, 1);
          will-change: transform;
        }
        .navfx:hover .navfx-underline {
          transform: scaleX(1);
        }

        /* reverse (unhover) automatically handled by transitions */
      `}</style>
    </>
  );
}
